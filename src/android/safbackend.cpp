...
constexpr int ANDROID_RESULT_OK = -1;
...
QList<DocEntry> queryUri(const QJniObject &uri)
{
    ...
    const auto projection = jni::stringArray({QStringLiteral("document_id"),
    ...
    int rename(const QString &from, const QString &to) override
    {
        ...
        if (fromParent != toParent) {
            ...
            if (!moved.isValid())
                return fail(EACCES);
            invalidate();
            currentId = resolve(toParent.isEmpty() ? nameOf(from)
...
{
    return QJniObject("android/content/Intent", "(Ljava/lang/String;)V",
                      QJniObject::fromString(action).object());
}

QJniObject openDocumentIntent(const QStringList &mimeTypes, bool multiple)
{
    auto intent = makeIntent(ACTION_OPEN_DOCUMENT);
    ...
    intent.callObjectMethod("setType", "(Ljava/lang/String;)Landroid/content/Intent;",
                            QJniObject::fromString(QStringLiteral("*/*")).object());
    if (!mimeTypes.isEmpty())
        intent.callObjectMethod("putExtra", "(Ljava/lang/String;[Ljava/lang/String;)Landroid/content/Intent;",
                                ...
                                jni::stringArray(mimeTypes).object());
    if (multiple)
        intent.callObjectMethod("putExtra", "(Ljava/lang/String;Z)Landroid/content/Intent;",
                                QJniObject::fromString(EXTRA_ALLOW_MULTIPLE).object(), jboolean(true));
    return intent;
}

QList<QUrl> resultUrls(const QJniObject &data)
{
    QList<QUrl> urls;
    if (!data.isValid())
        return urls;
    const auto clip = data.callObjectMethod("getClipData", "()Landroid/content/ClipData;");
    if (clip.isValid()) {
        const int count = clip.callMethod<jint>("getItemCount", "()I");
        for (int i = 0; i < count; ++i) {
            const auto item = clip.callObjectMethod("getItemAt", "(I)Landroid/content/ClipData$Item;", i);
            const auto uri = item.callObjectMethod("getUri", "()Landroid/net/Uri;");
            if (uri.isValid())
                urls.append(jni::toUrl(uri));
        }
    } else {
...
int nextRequestCode()
{
    static std::atomic<int> counter{0x5300}; // 'S' 'K'
...
{
    auto promise = std::make_shared<QPromise<FileSystem>>();
    QFuture<FileSystem> future = promise->future();
    promise->start();
    QtAndroidPrivate::startActivity(
        makeIntent(ACTION_OPEN_DOCUMENT_TREE), nextRequestCode(),
        [promise](int, int resultCode, const QJniObject &data) {
            QUrl rootUrl;
            if (resultCode == ANDROID_RESULT_OK && data.isValid()) {
                const auto uri = data.callObjectMethod("getData", "()Landroid/net/Uri;");
                if (uri.isValid()) {
                    jni::takePersistableUriPermission(uri);
                    rootUrl = jni::toUrl(uri);
                }
            }
...
QFuture<QUrl> platformPickFile(const QStringList &mimeTypes)
{
    ...
    promise->start();
    QtAndroidPrivate::startActivity(
        openDocumentIntent(mimeTypes, /*multiple=*/false), nextRequestCode(),
        [promise](int, int resultCode, const QJniObject &data) {
            const QList<QUrl> urls = resultCode == ANDROID_RESULT_OK ? resultUrls(data)
                                                                     : QList<QUrl>{};
            promise->addResult(urls.isEmpty() ? QUrl{} : urls.first());
            promise->finish();
        });
    return future;
}
...
QFuture<QList<QUrl>> platformPickFiles(const QStringList &mimeTypes)
{
    ...
    promise->start();
    QtAndroidPrivate::startActivity(
        openDocumentIntent(mimeTypes, /*multiple=*/true), nextRequestCode(),
        [promise](int, int resultCode, const QJniObject &data) {
            promise->addResult(resultCode == ANDROID_RESULT_OK ? resultUrls(data)
                                                               : QList<QUrl>{});
            promise->finish();
        });
    return future;
...
int platformOpenUrl(const QUrl &url, int flags)
{
    ...
    case O_RDONLY:
        ...
        break;
    case O_WRONLY:
        mode = (flags & O_TRUNC) ? QStringLiteral("wt")
                                 : (flags & O_APPEND) ? QStringLiteral("wa") : QStringLiteral("w");
        break;
    case O_RDWR:
        mode = (flags & O_TRUNC) ? QStringLiteral("rwt") : QStringLiteral("rw");
        break;
